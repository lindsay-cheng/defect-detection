...
class InspectionDashboard:
    ...
    def _setup_status_bar(self):
        ...
        tk.Label(
            status_frame, text="REC", bg="#4a7c59", fg="white", font=("Arial", 10, "bold"), padx=10
        ).pack(side=tk.LEFT)

        self.fps_label = tk.Label(
            status_frame, text="FPS: 0.0", bg="#4a7c59", fg="white", font=("Arial", 10)
        )
        self.fps_label.pack(side=tk.LEFT, padx=10)

        tk.Label(status_frame, text="|", bg="#4a7c59", fg="white").pack(side=tk.LEFT)

        self.inspected_label = tk.Label(
            status_frame, text="Inspected: 0", bg="#4a7c59", fg="white", font=("Arial", 10)
        )
        self.inspected_label.pack(side=tk.LEFT, padx=10)

        tk.Label(status_frame, text="|", bg="#4a7c59", fg="white").pack(side=tk.LEFT)

        self.fails_label = tk.Label(
            status_frame, text="Fails: 0", bg="#4a7c59", fg="white", font=("Arial", 10)
        )
    ...
    def _setup_inspection_panel(self, parent):
        ...
        for row, (title, value, fg_color, weight) in enumerate(labels):
            ...
            tk.Label(
                ...
                font=("Arial", 10),
                anchor="w",
            ).grid(row=row, column=0, sticky=anchor_sticky, pady=5)

            extra = {"wraplength": 280, "justify": "left"} if title == "Defect:" else {}
            value_label = tk.Label(
                info_frame, text=value, bg="#f5f5f5", fg=fg_color, font=font, anchor="w", **extra
            )
    ...
    def _create_button(self, parent, text, bg, fg="white", side=None, bordered=False):
        ...
        label = tk.Label(
            ...
            padx=20 if not bordered else 20,
            pady=5 if not bordered else 3,
        )
        label.pack()
    ...
    def show_stats(self, database):
        ...
        text = tk.Text(stats_window, bg="white", fg="black", font=("Courier", 10), padx=20, pady=20)
