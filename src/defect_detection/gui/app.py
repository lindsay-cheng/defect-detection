...
from defect_detection.gui.dashboard import InspectionDashboard
...
class DefectDetectionApp:
    ...
    def __init__(self, config: DetectorConfig | None = None, video_path: str = "assets/video5.mov"):
        ...
        self.detector = DetectionPipeline(config)
    ...
    def _detection_loop(self):
        ...
        try:
            reader = FrameReader(
                self.video_path,
                loop=True,
                on_loop=self.detector.reset_tracking_state,
            )
        except RuntimeError as e:
            log.error(str(e))
            self.root.after(0, self.dashboard.show_error, str(e))
            self.detection_running = False
            return
    ...
    def _export_callback(self):
        ...
        def export_task():
            from defect_detection.cli.db_utils import export_to_csv
...
def main() -> None:
    ...
    from defect_detection.storage.database import init_database
